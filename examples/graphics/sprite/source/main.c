...
int main() {
    ...
    // 1. Load into RAM Slot 0
    NF_LoadSpriteGfx("sprite/character", 0, 32, 32);
    NF_LoadSpritePal("sprite/character", 0);

    // 2. Transfer to Screen 1 (Bottom) VRAM from RAM Slot 0
    // Params: (Screen, RAM_Slot, VRAM_Slot, Keep_in_RAM)
    ...
    // 3. Create on Screen 1
    for(int i = 0; i < 3; i++){
        NF_CreateSprite(1, 0 + i, 0, 0, char_x + 30 * i, char_y + 30 * i);
        NF_SpriteFrame(1, 0 + i, 0 + i);
    }
    ...
    while(1) {
        ...
        // Update the OAM buffer for the Bottom screen
        NF_SpriteOamSet(1);
